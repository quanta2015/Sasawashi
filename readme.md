...
```
こんにちは、私は李陽てす。現在杭州師範大学で働いています。専門はコンピュータシステム構造で、ソフトウェア業界に従事して30年ぐらいになりました。フロントエンド、バックエンドの設計開発だけでなく、阿里雲/AWSのネットワーク運用サービスを提供するために、さまざまなシステムソフトウェアの開発設計に長けています。また、特に高効率ネットワークシステム、RPAロボット、低コード開発待ち、敏捷開発など、システムレベルのソリューションを提供することができます。後でEコマースサイトの例を示します。日本のビジネス会社の設計を参考に改版したもので、全体の作業に5日しかかかりませんでしたが、改版後は元のサイトの運用効率とユーザー体験が向上しました。
